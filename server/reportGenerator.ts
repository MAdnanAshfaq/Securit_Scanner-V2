...
export async function generatePDFReport(scanId: number): Promise<string> {
  try {
    ...
    for (const { vulnerability, analysis } of vulnerabilityAnalyses) {
      ...
      // Add technical proof of concept
      doc.moveDown();
      ...
      // Add example exploit code or request
      if (vulnerability.details) {
        doc.rect(50, doc.y, doc.page.width - 100, 100)
           .fill('#f8fafc');
        doc.text(vulnerability.details, 60, doc.y + 10, {
      ...
      const impacts = {
        [RiskLevel.HIGH]: 'Critical business impact with potential for data breach or system compromise',
        [RiskLevel.MEDIUM]: 'Moderate risk that could lead to service disruption or information disclosure',
        [RiskLevel.LOW]: 'Minor security concern with limited potential impact',
        [RiskLevel.INFO]: 'Informational finding that should be reviewed as part of security best practices'
      };

      doc.text(`Impact Level: ${vulnerability.severity}`);
      doc.text(`Business Impact: ${impacts[vulnerability.severity as RiskLevel]}`);
    }
